    ...
    for e in range(epochs):
        for images, labels in get_batches_fn(batch_size):
            feed_dict = {input_image: images,
                         ...
                         learning_rate: 1e-3,
                         keep_prob: 0.7}
            op, loss = sess.run([train_op, cross_entropy_loss], feed_dict=feed_dict)
        print("Epoch {}/{}, loss {}".format(e+1,epochs, loss))
...
def run():
    ...
    with tf.Session() as sess:
        ...
        logits, train_op, loss = optimize(output, correct_labels, learning_rate, num_classes)
        # TODO: Train NN using the train_nn function
        epochs = 30
        batch_size = 10
        train_nn(sess, epochs, batch_size, get_batches_fn, train_op, loss, input,
                 correct_labels, keep, learning_rate)
